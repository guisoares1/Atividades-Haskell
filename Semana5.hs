...
multiplique x n = if n == 0 then 0
                  else if n > 0 then x + multiplique x (n-1)
                  else negate (multiplique x (negate n))
...
potência x n = if n == 1 then x
               else if n > 0 then x * potência x (n-1)
               else negate (potência x (negate n))
...
-}
logBase2 :: Int -> Int
logBase2 n = if n== 1 then 0
             else 1 + logBase2 (n `div` 2)
...
{-
jogarForaAté :: String -> String -> String
jogarForaAté '[]' c = []
jogarForaAté (x:xs) c = if x==c then ([] + jogarForaAté (xs) c)
                        else [x + jogarForaAté (xs) c]
-}
