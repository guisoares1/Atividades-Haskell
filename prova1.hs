...
máximo :: [Int] -> Int
máximo [] = 0
máximo (x : xs) = if x > máximo xs then x
                  else máximo xs
...
raízes:: Equacao -> [Float]
raízes (a,b,c) | delta'>0 = [((-b + (delta' ** (1/2))) / (2*a)), ((-b - (delta' ** (1/2))) / (2*a))]
               | delta'==0 = [0]
               | otherwise = []    
               
               where delta' = delta(a,b,c)
...
valorFinal preco dataNasc dataHoje | idade <2 = ((preco/100)*15) 
                                   | idade <10 = ((preco/100)*40) 
                                   | idade >=70 = ((preco/100)*50) 
                                   | otherwise = preco
                                    where idade = calcidade dataHoje dataNasc

calcidade :: Data -> Data -> Int
calcidade (dia1, mes1, ano1) (dia2, mes2, ano2) = ano1 - ano2 
...
filtre (fil, num) l = if (igualfiltreMaior fil Maior) then maiores l num
                      else if (igualfiltreMenor fil Menor) then menores l num
                      else if (igualfiltreIgual fil Igual) then igual l num
                      else []
                      
igualfiltreMaior :: Filtro -> Filtro -> Bool
igualfiltreMaior Maior Maior = True

igualfiltreMenor :: Filtro -> Filtro -> Bool
igualfiltreMenor Menor Menor = True

igualfiltreIgual :: Filtro -> Filtro -> Bool
igualfiltreIgual Igual Igual = True

maiores :: [Int] -> Int -> [Int]
maiores [] num = []
maiores (x:xs) num = if x>num then concat [[x],maiores xs num]
                     else maiores xs num
                     
menores :: [Int]-> Int -> [Int]
menores [] num =[]
menores (x:xs) num = if x<num then concat [[x],menores xs num]
                     else menores xs num
                     
igual :: [Int]-> Int -> [Int]
igual [] num = []
igual (x:xs) num = if x==num then concat [[x] ,igual xs num]
                   else igual xs num
